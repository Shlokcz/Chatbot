...
class QueryRequest(BaseModel):
    query: str = Field(..., examples="How can I raise my voice against harassment?")
    username: str = Form(..., examples="userA")
...
class FeedbackRequest(BaseModel):
    message_id: str = Field(..., examples="123e4567-e89b-12d3-a456-426614174000")
    feedback: str = Field(..., examples="Like")
...
class ChatRequest(BaseModel):
    query: str = Field(..., examples="How can I raise my voice against harassment?")
    username: str = Field(..., examples="userA")
...
class ConversationLog(BaseModel):
    ...
    intent: str
    bot_response: str
    feedback: Optional[str] = Field(None, examples="Like")
...
async def log_conversation(message_id: str, username: str, user_query: str, bot_response: str, intent: str):
    """
    ...
    try:
        result = await conversation_collection.insert_one(conversation.dict())
...
async def upload_pdf(file: UploadFile = File(...), username: str = Form(..., examples={
            "userA_examples": {"username": "userA"},
            "userB_examples": {"username": "userB"}
        })):
    """
    ...
    if not user:
        logging.warning(f"Upload attempt by non-existent user: {username}")
    ...
    for chunk, embedding in zip(chunks, embeddings):
        ...
        vectors.append({
            'id': vector_id,
            'values': embedding,
            'metadata': {'text': chunk, 'username': username}  # Associate with user
        })
...
async def chat(request: ChatRequest):
    """
    Endpoint to handle chat queries associated with a specific user.
    """
    ...
    user = await users_collection.find_one({"username": username})
    if not user:
        logging.warning(f"Chat attempt by non-existent user: {username}")
        raise HTTPException(status_code=404, detail="User not found.")
    ...
    if not response['matches']:
        ...
        await log_conversation(message_id=message_id, username=username, user_query=query, bot_response=generated_text, intent=intent)
        return ChatResponse(message_id=message_id, answer=generated_text)
    
    # Concatenate the text of the matched documents for context
    retrieved_texts = [match['metadata']['text'] for match in response['matches'] if match['metadata'].get('username') == username]
    context = "\n".join(retrieved_texts)
    logging.info(f"Retrieved context for GPT: '{context[:100]}...'")  # Log the first 100 characters
    
    if not context.strip():
        generated_text = "Sorry, I couldn't find relevant information for your query."
        message_id = str(uuid.uuid4())
        await log_conversation(message_id=message_id, username=username, user_query=query, bot_response=generated_text, intent=intent)
        return ChatResponse(message_id=message_id, answer=generated_text)
    ...
    try:
        response_openai = await openai.ChatCompletion.acreate(
    ...
    # Log the conversation with intent to MongoDB, including message_id and username
    await log_conversation(message_id=message_id, username=username, user_query=query, bot_response=generated_text, intent=intent)
